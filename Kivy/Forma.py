...
class MenuScreen(Screen):
    def __init__(self, **kw):
        ...
        windows2 = FloatLayout()
        windows = GridLayout()
        windows.cols = 2
        windows.size_hint = (0.6, 0.7)
        windows.pos = (0,100)

        img = Image(source = "222.png",size_hint = (0.3,0.3),pos = (200,100), pos_hint= {'center_x:':0.5,'center_y':0.9})
        #mg.pos(0,100)

        windows2.add_widget(Button(text='Найшов', on_press=lambda x: set_screen('list_food'),size=(100, 100), size_hint=(None, None),pos_hint= {'center_x:':0.5,'center_y':0.5}))
        windows2.add_widget(Button(text='Шукаю', on_press=lambda x: set_screen('add_food'),size=(100, 100), size_hint=(None, None),pos_hint= {'center_x:':1,'center_y':0.3}))

        gre = Label(text = "Інформація про програму",pos = (200,100))
        windows2.add_widget(gre)
        windows2.add_widget(img)


        #self.add_widget(windows)
        self.add_widget(windows2)
